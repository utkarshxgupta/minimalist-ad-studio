...
/**
 * Returns the photograph's backdrop colour, or null when it does not have a
 * usable flat one: a cut-out image, a gradient or scene backdrop, a dark
 * ground, or a canvas the browser refuses to read. Null means keep the brand
...
export function sampleBackdrop(img: HTMLImageElement): string | null {
  ...
  try {
    ...
    if (!isFlat(corners)) return null;

    const avg = [0, 1, 2].map((c) => corners.reduce((sum, k) => sum + k[c], 0) / corners.length);
    if (luminance(avg[0], avg[1], avg[2]) < MIN_LUMINANCE) return null;

    return hex(avg[0], avg[1], avg[2]);
  } catch {
